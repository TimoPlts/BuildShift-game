...
## 2026-09-24 — Stage 1E — Controller Polish: Jump Buffer, Coyote Time & Fixed-Step Input

- **Scope:** Fixed the fixed-step jump-input edge case (a jump press was being consumed at render-frame time before any 60 Hz simulation step ran, so it was silently lost when the accumulator had not yet reached a full step) and added two standard responsiveness aids — a **jump buffer** and **coyote time** — as pure, shared logic that the future authoritative server can reuse. Also made input clearing (pointer-lock lost / blur / hidden / dispose) drop any buffered jump state so a stale press can never fire after gameplay input is disabled. No multiplayer, prediction/reconciliation, shooting, building, sprint/crouch/slide, or camera collision were added.
- **Root cause of the lost jump:** In Stage 1D `GameRuntime.renderFrame` called `consumeJumpRequested()` once at the top of the frame, then ran the accumulator loop. If the accumulator held less than one full `FIXED_DT`, zero simulation steps ran for that frame — and the already-consumed jump edge was gone forever. The press simply never reached a step.
- **Fix (input consumption):** The jump edge is now polled *inside* the fixed-step loop, via `PlayerController.update()`. `InputManager.pollJumpPressed()` is a latch that is only cleared when a fixed step actually polls it (or when input is cleared), so a press can never be consumed before a step runs and can never be lost. `GameRuntime.renderFrame` no longer touches the jump signal at all.
- **Jump buffer + coyote (pure, shared):** The launch *decision* lives in a new platform-independent `JumpController` class in `@buildshift/simulation` (no browser or physics types — only numbers and the lagged `grounded` flag). It is advanced **only by fixed steps**, so it is deterministic and frame-rate safe, and is directly reusable by the authoritative server step (see `docs/TECHNICAL_ARCHITECTURE.md` §7.4 / §18).
  - **Jump buffer:** a press is remembered for `jumpBufferTime`, so a press landing a fraction of a step before the character becomes grounded still launches on the next grounded step ("tap jump just before landing" feels responsive).
  - **Coyote time:** leaving the ground (walking off a ledge) still allows a jump for `coyoteTime` seconds, so a jump pressed just after losing ground is not rejected.
  - **Single-launch guarantee:** the buffer is consumed the moment a jump launches, so one press produces exactly one jump.
- **Files created:**
  - `packages/simulation/src/movement/jumpController.ts` — the `JumpController` class (`step(deltaSeconds, grounded, jumpPressed) -> boolean`, `reset()`), with the full timing decision and JSDoc.
- **Files changed:**
  - `packages/simulation/src/movement/types.ts` — added `JumpControllerConfig { jumpBufferTime; coyoteTime }`.
  - `packages/simulation/src/movement/stepVerticalMovement.ts` — the `jumpRequested` parameter now means "a jump should launch this step" (decided by `JumpController` upstream); the `grounded` flag is now used **only** for the downward-velocity clamp, not for jump gating. Core change: `if (jumpRequested) velocity = config.jumpSpeed;` (the old `&& grounded` condition was removed because the buffer/coyote decision already encodes eligibility).
  - `packages/simulation/src/index.ts` — exported `JumpController` and the `JumpControllerConfig` type.
  - `packages/simulation/src/simulation.test.ts` — updated the `stepVerticalMovement` tests (the old "rejects airborne" test is replaced by "applies jump speed whenever launch is requested", since the buffer/coyote gate now lives upstream) and added a new `describe("JumpController")` block (9 tests). Full suite now 26 tests.
  - `packages/game-config/src/physics.ts` — added `JUMP_INPUT_TIMING = { jumpBufferTime: 0.12, coyoteTime: 0.1 } as const`.
  - `packages/game-config/src/index.ts` — exported `JUMP_INPUT_TIMING`.
  - `apps/web/src/game/input/InputManager.ts` — `jumpRequested` → `jumpPressed` latch; `consumeJumpRequested()` → `pollJumpPressed()` (per-step, not per-frame); added an `inputCleared` latch and `consumeInputCleared()` (read per frame); `clearInput()` now also sets `inputCleared = true` so unlock / blur / hidden / dispose all signal the runtime.
  - `apps/web/src/game/player/PlayerController.ts` — now owns a `JumpController` (constructed with `JUMP_INPUT_TIMING`); `update(deltaSeconds, cameraYawRadians)` (the `jumpRequested` parameter was removed) polls `pollJumpPressed()`, advances `jumpController.step(...)`, and passes the launch decision to `stepVerticalMovement`; added `resetJumpState()` to clear buffered/coyote state.
  - `apps/web/src/game/GameRuntime.ts` — removed the top-of-frame `consumeJumpRequested()` call; added `if (this.inputManager.consumeInputCleared()) this.playerController.resetJumpState();` per frame; the accumulator loop now calls `playerController.update(FIXED_DT, cameraYaw)`.
- **Input-cleared handling:** `InputManager.clearInput()` (already wired to pointer-lock lost, window blur, document hidden, and dispose) latches `inputCleared = true`. The runtime polls it once per frame and calls `playerController.resetJumpState()` → `jumpController.reset()`, clearing the buffer and coyote window so a press captured right before gameplay input is disabled can never fire after it is re-enabled.
- **Tuned values:** `jumpBufferTime = 0.12 s`, `coyoteTime = 0.1 s` — standard, sub-frame windows; both live in `game-config` so they are tunable without touching logic.
- **Unit tests (26 total, all passing):** The `JumpController` describe block covers: immediate launch when grounded + press; no launch without a press; buffer a press so a later grounded step launches; buffered press expires after the buffer window; coyote-time grant after leaving the ground; coyote rejection after the window expires; a single press never double-jumps across multiple steps; `reset()` clears state; and a config-shape guard against divergence from the shared `JUMP_INPUT_TIMING`. The two `stepVerticalMovement` tests that depended on the old airborne-rejection semantics were updated to assert the new "launch is decided upstream" contract.
- **Local verification:** `pnpm typecheck` (all 5 projects passed), `pnpm build` (succeeded; only the known non-blocking Babylon chunk-size warning), and `pnpm test` (26/26 passed) all passed on this branch.
- **Browser verification (`pnpm dev`):** The page loads with one canvas and **zero console/runtime errors**; the scene, overlay ("Click to play … Space to jump … Esc to release"), and dev scene status all render. **Live gameplay (jump-buffer landing, coyote ledge jump, single-jump, Esc unlock) could not be driven synthetically**: this automation browser refuses the Pointer Lock API (`requestPointerLock()` never grants, so `pointerLockElement` stays `none`), and `InputManager` intentionally gates all gameplay input on pointer lock — the same, already-documented security behaviour from Stages 1C/1D. A manual click-to-play in a real browser is required to confirm the feel; the underlying timing logic is covered by the passing `JumpController` tests.
- **Important decisions:**
  - The jump *decision* was deliberately moved from the browser `PlayerController`/`stepVerticalMovement` into the shared `@buildshift/simulation` package so the client and the future authoritative server share one deterministic rule (per `TECHNICAL_ARCHITECTURE.md` §7.4 / §18), while `InputManager` remains browser-only and only *delivers* the press edge.
  - `stepVerticalMovement`'s `grounded` parameter was repurposed to do only the downward-velocity clamp; jump eligibility is no longer a property of the vertical integrator, keeping each unit single-responsibility.
- **Still unresolved:** Live, felt gameplay — especially the *feel* of the buffer and coyote windows — must be confirmed by a human in a real browser (the automation environment refuses pointer lock). The values `0.12 s` / `0.1 s` are reasonable defaults but should be re-tuned after a human playtest. Camera collision, sprint/crouch/slide, and the Stage 2 protocol/networking work remain intentionally deferred.

### Stage 1E follow-up (review fix) — coyote window must not enable an airborne second jump

- **Root cause:** `JumpController.step` refreshed `coyoteRemaining` to the full `coyoteTime` whenever `grounded` was true but did **not** invalidate it when a jump launched. So after a normal grounded jump (step 1: grounded launch), the next airborne step still saw `coyoteRemaining > 0`; a fresh airborne jump press within that window re-entered the `jumpEligible && pressValid` branch and returned `true` — an unintended second jump. Coyote is meant to cover walking/falling **off** a ledge, not to grant an extra jump right after a normal jump.
...
## 2026-09-24 — Stage 2A — Colyseus Server Foundation

- **Scope:** Added the first runnable multiplayer-server foundation without introducing player state, input messages, simulation, prediction/reconciliation, combat, building, matchmaking, authentication, or persistence. The existing Stage 1E client and shared gameplay packages remain unchanged.
- **Server architecture:** `apps/game-server` now creates `@colyseus/core` `Server` instances with `@colyseus/ws-transport`, registers one deliberately stateless `FoundationRoom`, exposes reusable create/start/shutdown helpers, and runs under native Node ESM with `.js` relative imports. The entry point owns idempotent SIGINT/SIGTERM handling and reports the requested/bound port and room registration.
- **Dependencies:** Final direct runtime dependencies are `@colyseus/core@0.18.16`, `@colyseus/ws-transport@0.18.3`, and `express@4.22.3`; `@colyseus/sdk@0.18.4` is test-only. Express remains direct because the installed WebSocket transport statically imports it and uses it for HTTP routing despite declaring it as an optional peer. The optional `msgpackr-extract@3.0.4` native build is explicitly denied (`allowBuilds: false`); a forced reinstall plus the SDK smoke tests verified Colyseus works through msgpackr's pure-JS path.
- **Integration fixes:** Replaced permissive `Number.parseInt()` port handling with a server-only `resolvePort` helper that accepts only complete base-10 integer strings from 1 through 65535 and otherwise falls through `GAME_SERVER_PORT` → `PORT` → `2567`. Added regression coverage for valid boundaries, whitespace, empty values, trailing garbage, decimals, exponent notation, zero, negatives, overflow, precedence, and defaulting.
- **Automated smoke testing:** A real `@colyseus/sdk` client test boots on an OS-assigned ephemeral port, joins/creates `foundation`, confirms one server-side client, joins a second client, verifies counts `2 → 1 → 0`, observes room disposal, and awaits graceful shutdown. The full suite passes: 2 files, 32/32 tests (27 Stage 1E simulation tests plus 5 Stage 2A server/config tests).
...
## 2026-09-24 — Stage 2B2 — Stateful Room and Browser Networking Foundation

- **Integration:** Merged the accepted server-state/input branch first and the browser-networking branch second onto `aa635e9`; both histories were preserved. Git combined the lockfile without conflict and retained direct `@colyseus/schema@^5.0.33` (server) and `@colyseus/sdk@^0.18.4` (web) dependencies.
- **Server:** The `foundation` room now synchronizes a real `MapSchema` keyed by `sessionId`. Each player exposes the accepted nested shape (`playerId`, capsule-centre `position.{x,y,z}`, `yaw`, `acknowledgedSequence`), starts at acknowledgement `-1`, and is removed on leave. `player:input` payloads are structurally validated through the shared protocol and then checked for a strictly increasing sequence; accepted inputs advance acknowledgement and yaw while position remains unchanged until Stage 2C. Malformed, duplicate, and stale inputs do not mutate state.
- **Client:** Added an isolated Colyseus SDK factory and page-lifetime foundation adapter using shared room/event constants, `VITE_GAME_SERVER_URL` with the `ws://localhost:2567` fallback, strict nested snapshot validation, and a React network-status overlay. The client does not synthesize player IDs or accept flat position fields. `sendPlayerInput` exists as a validated send boundary, but controls, prediction, reconciliation, interpolation, and authoritative movement remain intentionally unwired.
- **Live verification:** The compiled server and production web preview connected successfully with one browser client and a second real SDK client. The UI showed valid room/session IDs and player count `1 → 2 → 1`; synchronized entries used the nested contract with acknowledgement `-1`. Sequence `0` and `1` advanced acknowledgement/yaw without moving position, while duplicate, malformed, and stale frames caused no mutation. With the server unavailable, the local canvas/game still loaded and the network UI reported the failure without a page exception.
- **Validation:** `pnpm install`, all five workspace typechecks, workspace and package-specific server/web builds, `git diff --check`, and two full test runs pass. The combined suite is **119/119**: 29 protocol, 27 Stage 1E simulation, 5 Stage 2A server, 18 Stage 2B2 server, and 40 Stage 2B2 client tests. The web build retains only the known non-blocking large-chunk warning.
- **Remaining scope:** Live control-to-input wiring, server-authoritative movement, prediction, reconciliation, and interpolation belong to Stage 2C. Pointer-lock movement feel still requires manual verification in a real interactive browser.
